...
namespace Microservices.IsIdentifiable.Tests
{
    class NoChisInAnyColumnsConstraintTests
    {
        ...
        public void Test_SingleValue_IsChi(string testValue)
        {
            ...
            Assert.AreEqual($"Found chi in field {nameof(testValue)}",constraint.Validate(null,new []{testValue},new []{nameof(testValue)}));
        }
        ...
        {
            var constraint = new NoChisInAnyColumnsConstraint();
            Assert.IsNull( constraint.Validate(null, new[] { testValue }, new[] { nameof(testValue) }));
        }
