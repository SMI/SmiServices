...
namespace Microservices.DicomRelationalMapper.Tests.RunMeFirstTests
{
    ...
    public class RunMeFirstMongoServers
    {
        ...
        public void RabbitAvailable()
        {
            ...
            Assert.DoesNotThrow(()=> _=new RabbitMQBroker(rabbitOptions.CreateConnectionFactory(),nameof(RabbitAvailable)), $"Rabbit failed with the following configuration:{Environment.NewLine}{rabbitOptions}");
        }
    }
}
