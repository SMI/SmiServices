...
namespace Microservices.IdentifierMapper.Execution.Swappers
{
    ...
    /// we create a new value (depending on the derived class implementation), insert it into the database, and return it as the
    /// swapped value. Keeps a cache of swap values
    /// </summary>
    public abstract class ReplacementValueIdentifierSwapper : SwapIdentifiers
    {
        ...
        public ReplacementValueIdentifierSwapper()
        {
        ...
        public override string GetSubstitutionFor(string toSwap, out string reason)
        {
            ...
            lock (_oCacheLock)
            {
                ...
                var replacement = GetReplacementValue();
...
where not exists(select * 
                 from {0} 
                 where ({1} = '{3}'))
", _table.GetFullyQualifiedName(), _options.SwapColumnName, _options.ReplacementColumnName, toSwap, replacement);
    ...
    replacement);
                ...
                using (var con = _table.Database.Server.BeginNewTransactedConnection())
                {
                    ...
                    // value may not have been inserted.  Just because we don't have it in our cache doesn't mean that other people might
        ...
        private void CreateTableIfNotExists()
        {
            try
            {
                ...
                if (!_table.Exists())
                {
                    _logger.Info("Mapping table does not exist, creating it now");
                ...
                if (_table.Exists())
                    _logger.Info("Mapping table exists (" + _table + ")");
                else
            ...
            catch (Exception e)
            {
                ...
                throw new Exception("Error creating/checking substitution table on:" + Environment.NewLine + sb, e);
            }
        }
