        ...
        protected MicroserviceHost(GlobalOptions globals, bool loadSmiLogConfig = true)
        {
            ...
            if (loadSmiLogConfig)
            {
                string logConfigPath = !string.IsNullOrWhiteSpace(globals.FileSystemOptions.LogConfigFile)
                    ...
                    : Path.Combine(globals.CurrentDirectory, "Microservices.NLog.config");

                if (!File.Exists(logConfigPath))
                    throw new FileNotFoundException("Could not find the logging configuration in the current directory (Microservices.NLog.config), or at the path specified by FileSystemOptions.LogConfigFile");
